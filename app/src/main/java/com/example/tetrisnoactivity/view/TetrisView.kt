...
import com.example.tetrisnoactivity.GameActiviy
...
class TetrisView: View {
    ...
    private var activiy: GameActiviy? = null
            ...
            if(msg.what==0){
                if(owner.model != null){
                    if(owner.model!!.isGameOver()){
                        owner.model?.endGame()
                        Toast.makeText(owner.activiy, "Game over", Toast.LENGTH_LONG).show()
                    }
    ...
    fun setActivity(gameActiviy: GameActiviy){
        this.activiy = gameActiviy
    }
    ...
    private fun updateScore(){
        activiy?.tvCurrentScore?.text = "${model?.score}"
        activiy?.tvHighScore?.text = "${activiy?.appPreferences?.getHighScore()}"
    }
